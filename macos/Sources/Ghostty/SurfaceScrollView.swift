...
class SurfaceScrollView: NSView {
    ...
    override func mouseMoved(with: NSEvent) {
        // When the mouse is moved over the scrollbar area and it looks like the
        // user needs to be able to click and drag the scroller (because the
        // legacy style is preferred on the OS level), we flash the scrollbar.
